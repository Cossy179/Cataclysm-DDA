...
| z_level(`s`/`v`)    |  ✅   |   ❌  | g  | Return the z-level of a location, from -10 (deepest underground) to 10 (highest in the sky); ground level is 0.<br/>Argument is a location variable or special string (`u`, `npc`). `u` means your location. `npc` means NPC's location.<br/><br/>Examples:<br/>`"condition": { "math": [ "z_level('u') >= 0" ] }`<br/>`"condition": { "math": [ "z_level(_pos) == -1" ] }`|
