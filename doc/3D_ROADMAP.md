...
**Milestone punts** (still open in this phase): graffiti; debug overlays and item name-tag async text; color-block highlight overlays (auto-travel preview); mouse picking (the inverse projection in `visible_cell_bounds` is the starting point); colored light and night vision; true stair/ramp slopes; translucent fields (opaque slabs pending alpha-blend work in the triangle batch); animations routed through `tilecontext` (bullets/explosions don't appear); z-levels above the camera; detailed vehicle shapes; chunked mesh caching (geometry is re-emitted per frame — fine at current vertex volumes, revisit with caching keyed to submaps if profiling demands).
